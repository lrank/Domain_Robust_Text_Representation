...
import data_helpers
from text_cnn_cond import TextCNN
...
tf.flags.DEFINE_integer("num_tune_epochs", 50, "Number of tuning epochs")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        var_d = [var for var in all_var_list if 'domain' in var.name]
        assert( len(var_d) == 4)
        ...
        def get_indice_by_min_H(matrix):
            index = 0
            min_h = entropy( matrix[0] )
            for i in range(1, len(matrix) ):
                h = entropy( matrix[i] )
                if h < min_h:
                    min_h = h
                    index = i
            return np.argmax( matrix[index] )
        
        def dev_batch(x_batch, y_batch, d_batch):
            """
            Evaluates model on a dev set
            """
            # NUM_TRAINING_DOMAIN = 4 .. 20
            scores_list = []
            for i in range(4, 20):
                ...
                step, loss, scores, accuracy, d_l, d_a = sess.run(
                    [global_step, cnn.y_loss, cnn.y_scores_softmax, cnn.y_accuracy, cnn.domain_loss, cnn.domain_accuracy],
                    feed_dict)
                scores_list.append( scores )
            scores_list = np.stack(scores_list, axis = 1)
            cor = 0.
            for i in range( len(x_batch) ):
                ind = get_indice_by_min_H( scores_list[i] )
                if ind == np.argmax(y_batch[i]):
                    cor += 1
            accuracy = cor / len(x_batch)
